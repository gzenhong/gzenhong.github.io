...
啞鈴動作圖鑑 + PPL 課表 + 訓練紀錄。單一資料夾、純靜態、可安裝成手機 App。
...
改完檔案後，把 `sw.js` 裡的 `dumbbell-atlas-v1` 改成 `v2`（每次改都 +1），使用者下次開啟才會拿到新版，否則會讀到舊快取。

## 資料儲存

訓練紀錄存在瀏覽器的 `localStorage`，key 為 `dba:sessions`。
